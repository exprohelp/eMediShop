...
namespace eMediShop
{
    public delegate void ExProDragedItemEventHandler(object sender, ExProDragedEventArgs e);
    public class DragAndDropListView : ListViewCustom
    {
		#region Private Members
        public event ExProDragedItemEventHandler SearchUpdated;
		...
		protected override void OnDragDrop(DragEventArgs drgevent)
		{
			...
			if(m_previousItem != null) m_previousItem = null;
		    this.Invalidate();
			// call the base on drag drop to raise the event
			base.OnDragDrop(drgevent);
            TargetListView = base.Name;
            ExProDragedEventArgs ValuArgs = new ExProDragedEventArgs((ListViewItem)data.DragItems[0],TargetListView);
            ...
            if (_Result.Contains("Success"))
            {
                for (int i = 0; i < data.DragItems.Count; i++)
                {
                    ...
                    string s = newItem.SubItems[1].Text;
    ...
    public class ExProDragedEventArgs : System.EventArgs
    {
        private string target;
        private ListViewItem initItem;
        public ExProDragedEventArgs(ListViewItem lvi, string Target)
        {
            this.target = Target;
            this.initItem = lvi;
        }
