                    ...
                    if (((Conversion.Val(txtPayable.Text) > 0) || Conversion.Val(txtPayable.Text) < 0))
                    {
                        ...
                        switch (dr)
                        {
                            case DialogResult.Yes:
                                try
                                {
                                    //if (prescription_from == "IPD")
                                    //{
                                    //    _PayMode = "Credit";
                                    //}
                                    if (_CreditFlag == "Y")
                                        _PayMode = "Credit";
                                    else if (_CreditFlag == "N" && chkCash.Checked)
                                        _PayMode = "Cash";
                                    else if (_CreditFlag == "N" && chkSwipeCard.Checked)
                                        _PayMode = "Credit Card";
                                    else if (_CreditFlag == "N" && chkChq.Checked)
                                        _PayMode = "Cheque";
                                    ...
                                    p.payMode = _PayMode; p.payDetail = txtChqueNo.Text; p.login_id = GlobalUsage.Login_id;
        ...
        private void makeSale(string item_id, string masterkey, int qty)
        {
            ...
            if (dwr.message.Contains("Success"))
            {
                ...
                if (dsStock.Tables.Count > 0 && dsStock.Tables[2].Rows.Count > 0)
                {
                    ...
                    //p1.unit_id = GlobalUsage.Unit_id; p1.ipopno = _ipopno; p1.indent_no = _order_no; p1.item_id = item_id;
                    //p1.qty = Convert.ToInt32(qty);
                    //p1.prm_1 = "-"; p1.logic = "Issue"; p1.login_id = GlobalUsage.Login_id;
                    //datasetWithResult dwr1 = ConfigWebAPI.CallAPI("api/hospital/updateIndent", p1);
                    //string[] rr = dwr1.message.Split('|');
                    //int pending = Convert.ToInt16(rr[1]);
