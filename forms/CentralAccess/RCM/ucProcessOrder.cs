...
{
    public partial class ucProcessOrder : UserControl
    {
        ...
        private void loadOrder()
        {
            try
            {
                ...
                p1.Logic = "GET_ORDER";p1.dtFrom = "1900/01/01";
                datasetWithResult dwr1 = ConfigWebAPI.CallAPI("api/customerdata/rcmqueries", p1);
                ds = dwr1.result;

                cm1 p = new cm1();
                p.unit_id = GlobalUsage.Unit_id; p.Logic = "UnPosted"; p.prm_1 = "N/A"; p.login_id = GlobalUsage.Login_id;
                ...
                var Order = ds.Tables[0].AsEnumerable().Select(s => new
                {
                    ...
                    info = s.Field<string>("cust_name") + " [" + s.Field<string>("order_no") + "]",
        ...
        private void FillOrderDetail(string order_no)
        {
            ...
            try
            {
                ...
                lvOrder.Items.Clear();
                cm2 p2 = new cm2();
                p2.unit_id = GlobalUsage.Unit_id;p2.login_id = GlobalUsage.Login_id;p2.tran_id = order_no;
                p2.Logic = "ORDER_DETAIL"; p2.dtFrom = "1900/01/01";
                datasetWithResult dwr1 = ConfigWebAPI.CallAPI("api/customerdata/rcmqueries", p2);
                dsOrder = dwr1.result;
                ...
                if (dsOrder.Tables.Count > 0 && dsOrder.Tables[1].Rows.Count > 0)
                {
                    FillFamily(dsOrder.Tables[1].Rows[0]["card_no"].ToString());

                    txtRefBy.Text = dsOrder.Tables[1].Rows[0]["ref_by"].ToString();
                    cbxPtName.Text = dsOrder.Tables[1].Rows[0]["cust_name"].ToString();
                    if (dsOrder.Tables[1].Rows[0]["Home_Del"].ToString() == "Y")
                    {
                    ...
                    #region Prescription
                    if (dsOrder.Tables[1].Rows[0]["file_path"].ToString().Length > 15)
                    {
                        rbtn_prescription.Enabled = true;
                        rbtn_prescription.Tag = dsOrder.Tables[1].Rows[0]["file_path"].ToString();
                    }
                    else
                    {
                        rbtn_prescription.Enabled = false;
                    }
                    #endregion
                }
        ...
        private void FillFamily(string card_no)
        {
            cbxPtName.Items.Clear();
            DataSet ds;
            ds = GlobalUsage.healthcard_proxy.HealthCardQueries(out _result, GlobalUsage.Unit_id, "Cardmembers", "1900/01/01", "1900/01/01", card_no, "N/A", GlobalUsage.Login_id);
            if (ds.Tables[0].Rows.Count > 0)
            {
                //string[] pt = ds.Tables[0].Rows[0]["member_name"].ToString().Split('|');
                //foreach (string s in pt)
                foreach(DataRow dr in ds.Tables[0].Rows)
                {
                    //if (s.Trim().Length > 2) cbxPtName.Items.Add(s);
                    cbxPtName.Items.Add(dr["member_name"].ToString());
                }
                cbxPtName.SelectedIndex = 0;
            }
        ...
        protected void ReOrderPendingItems()
        {
            ...
            if (dsOrder.Tables.Count > 0)
            {
                if (dsOrder.Tables[0].Rows.Count > 0)
                {
                    ...
                    var Order = dsOrder.Tables[0].AsEnumerable().Select(s => new { OItemId = s.Field<string>("item_id"),
                        Item_name = s.Field<string>("item_name"), O_Qty = s.Field<decimal>("ordqty") });


                    var Sale = ItemSaleGrid.Items.Cast<ListViewItem>().GroupBy(x => x.Text).Select(l => 
                    new { SItemId = l.Key, sale_qty = l.Sum(y => Convert.ToDecimal(y.SubItems[9].Text)) });
                    var pendinglist = (from po in Order
                                       join c in Sale on po.OItemId equals c.SItemId into g
                                       from c in g.DefaultIfEmpty()
                                       select new
                                       {
                                           ItemId = po.OItemId,
                                           ItemName = po.Item_name,
                                           Bal_Qty = po.O_Qty - (c != null ? c.sale_qty : 0),
                                       }).Where(x => x.Bal_Qty > 0).OrderBy(z => z.ItemName);

                    if (pendinglist != null)
                    {
                        foreach (var pending in pendinglist)
                        {
                            ListViewItem lv = new ListViewItem(pending.ItemId);
                            lv.SubItems.Add(pending.ItemName);
                            lv.SubItems.Add(pending.Bal_Qty.ToString("###0"));
                            lvPending.Items.Add(lv);
                        }
                    }
                    #endregion
                    #region Reorder Process
                    foreach (ListViewItem lv in lvPending.Items)
                    {
                        string newmed = "-";
                        if (lv.Text.Substring(0, 1) == "T")
                            newmed = lv.SubItems[1].Text;

                      
                        p.card_no=_cardNo;p.unit_id=GlobalUsage.Unit_id;p.old_order_no= _order_no;
                        p.login_id=GlobalUsage.Login_id;p.item_id=lv.Text; p.new_med= newmed;
                        p.qty= Convert.ToInt32(lv.SubItems[2].Text);p.order_no= _neworder_no;
                        dwr = ConfigWebAPI.CallAPI("api/customerdata/RCMReOrderPendings", p);
                        _result = dwr.message;
                        ...
                        _neworder_no = r[1];
                        lblPending.Text = "Re-Order Number : " + _neworder_no;
                    }
                    //Order Completion
                    p.card_no = _cardNo; p.unit_id = GlobalUsage.Unit_id; p.old_order_no = _order_no;
                    p.login_id = GlobalUsage.Login_id; p.order_no = _neworder_no;p.delivery_date = "1900/01/01";
                    p.delivery_time = "-";p.remarks = "Re Order";p.ref_by = txtRefBy.Text;p.old_order_no = _order_no;
                    p.sale_inv_no = txtInvNo.Text;p.promo_flag = _promoTag;p.prm_1 = "N/A";p.logic = "REORDER_COM";
                    dwr = ConfigWebAPI.CallAPI("api/customerdata/RCMCompleteOrder", p);
                    if (dwr.message.Contains("Successfully"))
                    { lvOrder.Enabled = false; try { rgv_orders.Rows[_ordindex].Delete(); } catch (Exception ex) { } }
                    #endregion
                }
            }
        }
                        ...
                        case DialogResult.Yes:
                            try
                            {
                                ...
                                if ((string)rtBtn.Tag == "HD")
                                    ...
                                    p.gstn_no = "-"; p.HealthCardNo = _cardNo; p.Hosp_Cr_No = "-"; p.Hosp_IPOP_No = "-";
                                    p.rcmOrderNo = _order_no; p.hd_flag = "N"; p.refBy = txtRefBy.Text;
                                    p.ref_name = txtRefBy.Text;
                                    p.payMode = "Cash"; p.payDetail = "-"; p.login_id = GlobalUsage.Login_id;
                                ...
                                if (!dwr.message.Contains("Successfully"))
                                {
                                        ...
                                        c.order_no = _order_no;c.remarkfrom = "Manager";c.remarks = "Cash Memo No. " + txtInvNo.Text + " of Order No {" + _order_no + "}";
                                        c.login_id = GlobalUsage.Login_id;p.Sale_Inv_No = txtInvNo.Text;
                                        dwr = ConfigWebAPI.CallAPI("api/customerdata/InsertOrderRemark", p);
                                        _result = dwr.message;
        ...
        {
            Cursor.Current = Cursors.WaitCursor;
            eMediShop.forms.CentralAccess.RCM.order_remarks UseForm = new order_remarks(_order_no);
            UseForm.Show();
            Cursor.Current = Cursors.Default;
        }
