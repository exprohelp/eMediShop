...
namespace eMediShop
{
    public partial class SaleMaster : Telerik.WinControls.UI.RadForm
    {
        ...
        private void SaleMaster_Load(object sender, EventArgs e)
        {
            ...
            if (GlobalUsage.Unit_id == "CH-00050")
            { lblFlexi.Enabled = true; rtb_DisAmt.Enabled = true; rbtnzDisApply.Enabled = true; }
            else
            { lblFlexi.Enabled = false; rtb_DisAmt.Enabled = false; rbtnzDisApply.Enabled = false; }
                        ...
                        if (e.KeyCode == Keys.Return && (txtQty.Text != "0" && Config.isNumeric(txtQty.Text, System.Globalization.NumberStyles.Integer)))
                        {
                            ...
                            try
                            {
                                if (_Lock_Flag == "N")
                                {
                                    ...
                                    p.card_no = txtCardNo.Text; p.card_level = txtCardStatus.Text; p.discountPercent = _discountPercntage; p.sold_qty = Convert.ToDouble(txtQty.Text);
                                    p.old_sale_inv_no = GlobalUsage.Old_Sale_Inv_No; p.gstn_no = txtGSTN_No.Text; p.hosp_cr_no = "-"; p.hosp_ipop_no = "-";
                                    p.Cur_sale_inv_no = txtInvNo.Text; p.DiscountLogic = "New"; p.order_no = "N/A"; p.promo_flag = "N"; p.login_id = GlobalUsage.Login_id;
        ...
        protected void Fill_ItemSalesGrid(string Sale_Inv_No, DataSet ds)
        {
            try
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    //this.tglSwitch.ValueChanged -= new EventHandler(this.tglSwitch_ValueChanged);
                    //if (ds.Tables[0].Rows[0]["isLocked"].ToString() == "True")
                    //    tglSwitch.Value = false;
                    //else
                    //    tglSwitch.Value = true;

                    //if (ds.Tables[0].Rows[0]["isLocked"].ToString() == "True" && GlobalUsage.Posting_Rights == "Y" && (txtInvNo.Text.Contains("New") || txtInvNo.Text.Contains("E")))
                    //    btn_print.Enabled = true;
                    //else
                    //    btn_print.Enabled = false;
                    //this.tglSwitch.ValueChanged += new EventHandler(this.tglSwitch_ValueChanged);
        ...
        protected void ProcessDiscount(string item_id, string master_key_id, string HealthCardNo, Int32 SoldQty, string inv_no)
        {
            ...
            p.counter_id = GlobalUsage.CounterID; p.computer_id = GlobalUsage.computerName; p.Sale_Type = "Walk-In";
            p.cust_name = txtPatientName.Text; p.prescribedBy = txtPrescribedBy.Text; p.refCode = _refCode;
            p.item_id = item_id; p.master_key_id = master_key_id;
            p.card_no = HealthCardNo; p.card_level = txtCardStatus.Text; p.discountPercent = _discountPercntage; p.sold_qty = SoldQty;
            p.old_sale_inv_no = GlobalUsage.Old_Sale_Inv_No; p.gstn_no = txtGSTN_No.Text; p.hosp_cr_no = "-"; p.hosp_ipop_no = "-";
            p.couponCode = GlobalUsage.couponCode;
            p.Cur_sale_inv_no = inv_no; p.DiscountLogic = "ReCal"; p.order_no = "N/A"; p.promo_flag = "N"; p.login_id = GlobalUsage.Login_id;
        ...
        private void btn_print_Click(object sender, EventArgs e)
        {
            ...
            if (radioCash.Checked)
            {
                ...
                else if (chkChq.Checked)
                    _PayMode = "Cheque";
                else
                    _PayMode = "Cash";
            }
            if (chkChq.Checked && txtPaymentDetail.TextLength < 5)
            {
                MessageBox.Show("Type Detail of Cheque.", "ExPro Help", MessageBoxButtons.OK);
                txtPaymentDetail.Focus(); return;
            }
            else if (chkSwipeCard.Checked && txtPaymentDetail.TextLength < 5)
            {
                MessageBox.Show("Type Detail of Swipe Card.", "ExPro Help", MessageBoxButtons.OK);
                txtPaymentDetail.Focus(); return;
            }
            ...
            if (txtInvNo.Text.Substring(0, 1) == "E" && itemCont > 0 && txtPrescribedBy.Text.Length > 4)
            {
                    ...
                    switch (dr)
                    {
                        case DialogResult.Yes:
                            try
                            {
                                ...
                                p.gstn_no = "-"; p.HealthCardNo = txtCardNo.Text; p.Hosp_Cr_No = "-"; p.Hosp_IPOP_No = "-";
                                p.rcmOrderNo = _orderNo; p.hd_flag = (chkHomeDelivery.Checked) ? "Y" : "N"; p.refBy = _refCode;
                                p.ref_name = txtPrescribedBy.Text;
                                p.payMode = _PayMode;
                                p.swipeMacNo = "-";
                                p.payDetail = txtPaymentDetail.Text;
                                ...
                                else
                                {
                                    ...
                                    if (txtCardNo.Text.Length > 5 && s[2] == "N")
                                    {
                                        ipCardUpgrade pc = new ipCardUpgrade();
                                        pc.healthCardNo = txtCardNo.Text; pc.unit_id = p.unit_id; pc.sale_inv_no = txtInvNo.Text;
                                        pc.billAmt = txtNetValue.Text; pc.sale_Date = DateTime.Now.ToString("yyyy-MM-dd");
                                        ...
                                        if (dwrCard.Msg.Contains("Success"))
                                        {
                                            ...
                                            p1.Logic = "sale_master:cardUpgrd_Flag"; p1.prm_1 = "Y"; p1.tran_id = txtInvNo.Text;
        ...
        //protected string CheckReturnMemo()
        //{

        //    string result = "Passed";
        //    if (txtQty.Text.Length > 0)
        //    {
        //        if (txtOldCashMemoNo.Text.Length > 4 && Conversion.Val(txtQty.Text) < 0 && chkReturnMemo.Checked)
        //        {
        //            cm3 p = new cm3();
        //            p.unit_id = GlobalUsage.Unit_id; p.Logic = "CheckSalesReturnEntry"; p.prm_1 = _MasterKeyId; p.prm_2 = txtOldCashMemoNo.Text;
        //            p.prm_3 = "-"; p.prm_num = Convert.ToInt32(txtQty.Text); p.login_id = GlobalUsage.Login_id;
        //            datasetWithResult dwr = ConfigWebAPI.CallAPI("api/sales/retailstorequeries", p);
        //            if (_result.Contains("NO"))
        //            {
        //                if (txtOldCashMemoNo.Text != "Audit")
        //                    result = "Product Not Sold On Given Cash Memo No?..";
        //                else
        //                    result = "Return Quantity is Greater Than Sold Quantity?..";
        //            }
        //        }
        //        else if (Conversion.Val(txtQty.Text) > _Qty)
        //        {
        //            result = "Only " + _Qty.ToString("##") + " Quantity Can Be Sold Against This Selection";
        //        }
        //    }
        //    return result;
        //}
        ...
        private void chkCash_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                _PayMode = "Cash";
                txtPaymentDetail.Enabled = false;
        ...
        private void chkSwipeCard_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                _PayMode = "Credit Card";
                //txtPaymentDetail.Location = new Point(87, 380);
                //txtPaymentDetail.Size = new Size(168, 29);

                txtPaymentDetail.Enabled = true;
                chkChq.Checked = false;
            ...
            //else
            //{
            //    txtPaymentDetail.Location = new Point(6, 380);
            //    txtPaymentDetail.Size = new Size(250, 29);
        ...
        //{
        //    if (dtSwipeMacInfo.Rows.Count == 0)
        //    {
        //        cm1 p = new cm1();
        //        p.unit_id = GlobalUsage.Unit_id; p.Logic = "SwipeMachines"; p.prm_1 = "-"; p.prm_2 = "-"; p.prm_3 = "-";
        //        p.login_id = GlobalUsage.Login_id;
        //        datasetWithResult dwr = ConfigWebAPI.CallAPI("api/sales/retailstorequeries", p);
        //        dtSwipeMacInfo = dwr.result.Tables[0];

        //        foreach (DataRow dr in dtSwipeMacInfo.Rows)
        //        {
        //            RadListDataItem dataItem = new RadListDataItem();
        //            dataItem.Text = dr["bank_name"].ToString();
        //            this.rddlmacInfo.Items.Add(dataItem);
        //        }
        //        rddlmacInfo.SelectedIndex = 0;
        //    }
        //}
        ...
        private void chkST_Counter_CheckedChanged(object sender, EventArgs e)
        {
            if (chkST_Counter.Checked)
            {
                _SaleType = "N";
        ...
        private void chkHomeDelivery_CheckedChanged_1(object sender, EventArgs e)
        {
            if (chkHomeDelivery.Checked)
            {
                _SaleType = "Y";
                chkST_Counter.Checked = false;
            }
        ...
        private void rbtnzDisApply_Click(object sender, EventArgs e)
        {
            try
            {
                Cursor.Current = Cursors.WaitCursor;

                pm_FlexiDiscount p = new pm_FlexiDiscount();
                p.unit_id = GlobalUsage.Unit_id; p.sale_inv_no = txtInvNo.Text; p.FlexiDiscountAmt = Convert.ToInt16(rtb_DisAmt.Text); p.login_id = GlobalUsage.Login_id;
                datasetWithResult dwr = ConfigWebAPI.CallAPI("api/sales/FlexiDiscount", p);
                _holdFlag = "Y";
                Fill_ItemSalesGrid(txtInvNo.Text, dwr.result);
                rtb_DisAmt.Text = "";
                //ds.Dispose();
        ...
        private void tglSwitch_ValueChanged(object sender, EventArgs e)
        {
            //try
            //{
            //    Cursor.Current = Cursors.WaitCursor;
            //    if (ItemSaleGrid.Items.Count > 0)
            //    {
            //        int act = (tglSwitch.Value == true) ? 0 : 1;

            //        if(act==1 && GlobalUsage.Posting_Rights=="Y" && (txtInvNo.Text.Contains("New") || txtInvNo.Text.Contains("E")))
            //            btn_print.Enabled = true;
            //        else
            //            btn_print.Enabled = false;
            ...
            //        pm_LockSalesInv p = new pm_LockSalesInv();
            //        p.unit_id = GlobalUsage.Unit_id; p.login_id = GlobalUsage.Login_id;
            //        p.sale_inv_no = txtInvNo.Text; p.action = act; p.SystemName = GlobalUsage.computerName;
            //        datasetWithResult dwr = ConfigWebAPI.CallAPI("api/sales/LockSalesInvoice", p);
            //        if (!dwr.message.Contains("Success"))
            //        {
            //            MessageBox.Show(dwr.message, "ExPro Help", MessageBoxButtons.OK);
            //            this.tglSwitch.ValueChanged -= new EventHandler(this.tglSwitch_ValueChanged);
            //            tglSwitch.Value = false;
            //            this.tglSwitch.ValueChanged += new EventHandler(this.tglSwitch_ValueChanged);
