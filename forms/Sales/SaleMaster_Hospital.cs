...
namespace eMediShop.Hospital.Sales
{
    public partial class SaleMaster_Hospital : Telerik.WinControls.UI.RadForm
    {
        ...
        string _MasterKeyId = string.Empty; int _retPermitQty = 0;
        ...
        protected void Fill_Fin_Info(DataTable dt)
        {
            ...
            if (dr["sale_inv_no"].ToString().Contains("E"))
                ...
                _Lock_Flag = "Y";
            if (dr["pay_mode"].ToString().ToUpper() == "CR" || dr["pay_mode"].ToString().ToUpper() == "CREDIT")
            {
                _PayMode = "Credit";
                radioCredit.Checked = true; radioCash.Checked = false;
                rgb_payoptions.Enabled = false;
            }
            else
            {
                _PayMode = "Cash";
        ...
        protected void ProcessDiscount(string item_id, string master_key_id, string HealthCardNo, Int32 SoldQty, string inv_no)
        {
            ...
            p.cust_name = txtPatientName.Text; p.prescribedBy = txtPrescribedBy.Text; p.refCode = _refCode;
            p.item_id = item_id; p.master_key_id = master_key_id; p.couponCode = GlobalUsage.couponCode;
