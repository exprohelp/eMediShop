...
namespace eMediShop.Substitute
{
    public partial class UploadSubstitute_Doc : Telerik.WinControls.UI.RadForm
    {
        ...
        private void dgItemName_CommandCellClick(object sender, Telerik.WinControls.UI.GridViewCellEventArgs e)
        {
            try
            {
                ...
                if (chkImages.Checked)
                {
                    ...
                    p.item_id = _Itemid;
                    string response=ConfigWebAPI.CallAPIObject("api/products/SubstituteGetMultipleImages", p);
                    imageList=JsonConvert.DeserializeObject<byte[][]>(response, new JsonSerializerSettings
                    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txtSearch.Text.Length > 0)
                {
                    ...
                    dgItemName.DataSource = dwr.result.Tables[0];
                }
            }
            catch (Exception ex) { MessageBox.Show(ex.Message); }
            finally { Cursor.Current = Cursors.Default; }
        ...
        private void uploadSubstitute_Control1_Load(object sender, EventArgs e)
        {
