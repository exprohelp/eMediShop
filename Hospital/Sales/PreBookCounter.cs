...
{
    public partial class PreBookCounter : Telerik.WinControls.UI.RadForm
    {
        string _result = string.Empty; static IHubProxy _hub;
        DataSet _ds = new DataSet();
        public PreBookCounter()
        {
            InitializeComponent();
        }

        private void PreBookCounter_Load(object sender, EventArgs e)
        {
            //try
            //{
        ...
        {
            cm2 p = new cm2();
            p.unit_id = GlobalUsage.Unit_id; p.Logic = "PreBookCounter"; p.dtFrom = "1900/01/01";
            p.dtTo = "1900/01/01";
        ...
        private void rgv_Pending_CommandCellClick(object sender, Telerik.WinControls.UI.GridViewCellEventArgs e)
        {
            try
            {
                ...
                if (e.Row.Cells["sale_type"].Value.ToString() == "Walk-In")
                {
                    ...
                    eMediShop.sales.SaleMaster obj = new eMediShop.sales.SaleMaster("Pre-Book Counter",e.Row.Cells["estimate_no"].Value.ToString());
                ...
                {
                    DisplayTokenNo(e);
                    SaleMaster_Hospital obj = new SaleMaster_Hospital("Pre-Book Counter", e.Row.Cells["hosp_cr_no"].Value.ToString(), e.Row.Cells["estimate_no"].Value.ToString());
                    obj.CashCounterUpdated += new CashCounterEventHandler(Update_CashCounterUpdated);
                    obj.Owner = this;
                    obj.Show();
                }
